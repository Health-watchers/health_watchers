...
export async function down(db: Db): Promise<void> {
  await db
    .collection('labresults')
    .dropIndex('labresults_text_search')
    .catch(() => {});
  await db
    .collection('labresults')
    .dropIndex('labresults_clinicId_patientId_orderedAt')
    .catch(() => {});
  await db
    .collection('labresults')
    .dropIndex('labresults_clinicId_status_orderedAt')
    .catch(() => {});
  await db
    .collection('appointments')
    .dropIndex('appointments_clinicId_patientId_status')
    .catch(() => {});
  await db
    .collection('appointments')
    .dropIndex('appointments_clinicId_doctorId_status_scheduledAt')
    .catch(() => {});
  await db
    .collection('notifications')
    .dropIndex('notifications_userId_createdAt')
    .catch(() => {});
  await db
    .collection('notifications')
    .dropIndex('notifications_userId_isRead_createdAt')
    .catch(() => {});
  await db
    .collection('auditlogs')
    .dropIndex('auditlogs_clinicId_timestamp')
    .catch(() => {});
  await db
    .collection('auditlogs')
    .dropIndex('auditlogs_userId_timestamp')
    .catch(() => {});
  await db
    .collection('auditlogs')
    .dropIndex('auditlogs_action_timestamp')
    .catch(() => {});
  await db
    .collection('patients')
    .dropIndex('patients_clinicId_isActive_createdAt')
    .catch(() => {});
}
