...
export async function up(db: Db): Promise<void> {
  ...
  await db
    .collection('consents')
    .createIndex({ formVersion: 1 }, { sparse: true, name: 'formVersion_1' });
}

export async function down(db: Db): Promise<void> {
  await db
    .collection('consentforms')
    .dropIndex('clinicId_1_type_1_version_1')
    .catch(() => {});
  await db
    .collection('consentforms')
    .dropIndex('clinicId_1_type_1_effectiveDate_-1')
    .catch(() => {});
  await db
    .collection('consents')
    .dropIndex('formVersion_1')
    .catch(() => {});
}
