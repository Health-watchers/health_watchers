    ...
    <div className="space-y-3">
      {appointments.map((apt) => {
        ...
        return (
          ...
          >
            <div
              className={`flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full ${getStatusColor(apt.status)} text-sm font-bold`}
            >
            ...
            {isUpcoming && apt.status === 'scheduled' && (
              <div className="text-warning-700 dark:text-warning-300 flex-shrink-0 text-xs font-medium">
                Confirm
              </div>
            )}
          </div>
